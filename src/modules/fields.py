...
from modules.utils.utils_calib import FramebyFrameCalib, pan_tilt_roll_to_orientation, keypoint_aux_world_coords_2D, keypoint_world_coords_2D, line_world_coords_3D
from modules.utils.utils_heatmap import get_keypoints_from_heatmap_batch_maxpool, get_keypoints_from_heatmap_batch_maxpool_l, complete_keypoints, coords_to_dict
...
def visualize_field_and_players(detections: sv.Detections, color_lookup: np.ndarray, H_inv, output_path) -> np.ndarray:
    ...
    TEAM_COLORS = {'Team A': '#ff1493', 'Team B': '#00bfff'}

    # Draw player coordinates
    bbox_centers = detections.get_anchors_coordinates(sv.Position.BOTTOM_CENTER)
    coords_list = project_to_field_map(bbox_centers, H_inv)

    position = []  # Nama kunci untuk frame saat ini
    for detection_idx, (x, y) in enumerate(coords_list):
        ...
        if color.as_hex() == '#ff1493':
            team_sides = 'left'
        elif color.as_hex() == '#00bfff':
            team_sides = 'right'
        elif color.as_hex() == '#ffd700':
            team_sides = 'ref'
        else:
            team_sides = 'unknown'

        position.append({
            'player_idx': detection_idx,
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Keypoints', markerfacecolor='black', markersize=5, alpha=0.3),
        Line2D([0], [0], marker='o', color='w', label='Team A', markerfacecolor=TEAM_COLORS['Team A'], markersize=10),
        Line2D([0], [0], marker='o', color='w', label='Team B', markerfacecolor=TEAM_COLORS['Team B'], markersize=10)
    ]
 

    # Draw the key points
    keypoints_x, keypoints_y = zip(*keypoint_world_coords_2D)
    ax.scatter(keypoints_x, keypoints_y, color='black', label='Keypoints', s=10, alpha=0.3, zorder=10)
    ...
    ax.set_title("Football Field with keypoints and lines")
    #ax.legend()
    ...
    buf.seek(0)
    img_np = Image.open(buf)
    img_rgb = np.array(img_np.convert("RGB"))
